...
# for the plugins to be correctly set up and used withing the neovim configuration.
...
let
  ...
  plugins = rec {
    ...
    maybe_lazy =
      Dict.filter (_: v: !(Dict.member "lazy" v) || Nix.isA "bool" v.lazy)
      plugin-setups;

    # These are the plugins that will be dynamically loaded
    # based on a events and patterns.
    lazy = Dict.filter (_: v: Dict.member "lazy" v && Nix.isA "set" v.lazy)
      plugin-setups;
  };
  ...
  setup-code = name: plugin:
    if builtins.typeOf plugin != "set" then
      ...
      builtins.abort "PLUGIN DOES NOT HAVE THE SETUP ATTRIBUTE FIELD: '${
        String.toString plugin
      }'"
  ...
  setup-file = name: plugin:
    ...
    if nilm.Nix.isA "string" plugin then
      ...
      cp ${
        pkgs.writeText "${name}.lua" (setup-code name plugin)
      } $out/${app-name}/${
        Nix.orDefault (!(Dict.getOr "urgent" false plugin)) "after/"
      }plugin/;
    '';
  ...
  copy-source = { src, name ? "", opt ? false }:
    "cp -R ${src} $out/${app-name}/pack/${app-name}-plugins/${
      if opt then "opt" else "start"
    }/${Nix.orDefault opt (name + "/")};";
  ...
  compile-maybe-lazy-plugin = name: plugin:
    ...
    assert builtins.typeOf name == "string";
    let
      copy-src-cmd = copy-source {
        src = Dict.get name plugins.sources;
  ...
  compile-plugins-generic = mapfn:
    nilm.Basics.compose [ (String.join "\n") Dict.values (Dict.map mapfn) ];

  # The combined shell commands (string) to 'install' all the maybe-lazy-plugins.
  compile-maybe-lazy-plugins =
    compile-plugins-generic compile-maybe-lazy-plugin;

  # The combined shell commands (string) to 'install' all the raw-plugins.
  compile-raw-plugins =
    compile-plugins-generic (_: src: copy-source { inherit src; });
  ...
  compile-lazy-plugin-set = set:
    if builtins.typeOf set != "set" then
      builtins.abort
      "COMPILE-LAZY-PLUGIN-SET SET IS NOT A SET: '${String.toString set}'"
    else
      let lazy0 = (Tuple.second (List.get 0 (Dict.toList set))).lazy;
      in ''
        vim.api.nvim_create_autocmd(${lua.toLua lazy0.events}, {
          ${
            if Dict.member "pattern" lazy0 then
              "pattern = ${lua.toLua lazy0.pattern},"
            else
              ""
          }
          callback = function(event)
            ${
              nilm.Basics.pipe set [
                (Dict.map (name: args: "vim.cmd([[packadd ${name}]]);"))
                Dict.values
                (String.join "\n")
              ]
            }
            ${
              nilm.Basics.pipe set [
                (Dict.map (name: args:
                  if nilm.Nix.isA "bool" args.setup && !args.setup then
                    ""
                  else ''
  ...
  group-lazy-plugins = name: plugin: acc:
    let
      pattern = if Dict.member "pattern" plugin.lazy then
        if Nix.isA "list" plugin.lazy.pattern then
          ...
          builtins.abort
          "lazy.pattern was given a value that is not a String or a List of Strings."
      else
        "";

      key = (String.concat (List.sort plugin.lazy.events)) + pattern;
    in Dict.upsert key (Dict.insert name plugin) { ${name} = plugin; } acc;

  compiled-lazy-plugins = let
    events-file = nilm.Basics.pipe plugins.lazy [
      (Dict.foldl group-lazy-plugins { })
      (Dict.map (_: compile-lazy-plugin-set))
      Dict.values
      (String.join "\n")
      (pkgs.writeText "lazy-plugins.lua")
    ];

    copy-source-cmds = compile-plugins-generic (name: plugin:
      copy-source {
        src = Dict.get name plugins.sources;
        opt = true;
        inherit name;
